...
/**
 * Creates a small downloadable package for a Home module/service.
 * The package contains a README describing the selected module.
 */
export async function downloadModulePackage(
  ...
  const zip = new JSZip();
  const safeId = serviceId.replace(/[^a-zA-Z0-9_-]/g, "-").toLowerCase() || "module";
  zip.file(
    "README.md",
    `# ${title}\n\nService ID: ${serviceId}\n\n${description}\n\nGenerated by ANANT VR Super Platform.\n`
  );
  const blob = await zip.generateAsync({ type: "blob", compression: "DEFLATE" });
  saveAs(blob, `ANANT-VR-${safeId}.zip`);
}
